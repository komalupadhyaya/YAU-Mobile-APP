...
class UniformController {
  ...
  // Batch delete uniform orders (Admin only)
  async batchDeleteUniformOrders(req, res) {
    try {
      ...
      if (!orderIds || !Array.isArray(orderIds) || orderIds.length === 0) {
        return res.status(400).json({
          success: false,
          message: 'An array of orderIds is required for batch deletion.',
        });
      }

      console.log('🗑️ Admin batch deleting uniform orders:', orderIds.length, 'orders');

      const result = await uniformService.batchDeleteUniformOrders(orderIds);

      res.status(200).json({
        success: true,
        message: `Successfully deleted ${result.deletedCount} uniform orders.`,
        data: result,
      });
      ...
      res.status(500).json({
        success: false,
        message: 'Failed to batch delete uniform orders.',
        error: process.env.NODE_ENV === 'development' ? error.message : undefined,
      });
    }
  }
