    ...
    <div className="space-y-8 pb-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        {schedules.map((schedule) => (
          <Card key={schedule.id} className={`group hover:shadow-2xl transition-all hover:-translate-y-1 relative border-none shadow-sm ${selectedIds.includes(schedule.id) ? 'ring-2 ring-indigo-600' : ''}`}>
             <div className="flex justify-between items-start mb-6">
                ...
                <Badge variant="secondary" className="px-3 py-1 font-black uppercase text-[10px] tracking-widest">{schedule.sport}</Badge>
                 <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                   <Button variant="ghost" size="sm" className="p-2 h-auto" onClick={() => openEditModal(schedule)}><Edit2 size={16} className="text-gray-400 dark:text-white/40 hover:text-indigo-600" /></Button>
                 </div>
              </div>
              ...
              <div className="absolute -bottom-3 right-6">
                 <Badge variant="info" className="bg-white dark:bg-indigo-900 text-indigo-600 dark:text-white border-none shadow-lg h-7 px-4 font-black uppercase text-[10px] tracking-widest">
                    {schedule.grade_band}
                 </Badge>
              </div>
          </Card>
