...
export const getGradeBand = (grade: string): string | null => {
  ...
  if (!isNaN(gradeNum)) {
    if (gradeNum <= 1) return 'Band1';
    if (gradeNum <= 3) return 'Band2';
    if (gradeNum <= 5) return 'Band3';
    if (gradeNum <= 8) return 'Band4';
  }
  
  // Fallback to string matching for grades like "Kindergarten", "Pre-K"
  if (g.includes('k') || g.includes('kindergarten') || g.includes('pre')) return 'Band1';
  if (g.includes('1')) return 'Band1';
  if (g.includes('2') || g.includes('3')) return 'Band2';
  if (g.includes('4') || g.includes('5')) return 'Band3';
  if (g.includes('6') || g.includes('7') || g.includes('8')) return 'Band4';
